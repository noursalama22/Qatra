  ...
  useEffect(() => {
    ...
    import("leaflet").then(L => {
      ...
      drivers.forEach(driver => {
        ...
        marker.on("click", () => { setSelected(driver as DriverFeature); setSelectedType("driver"); });
        marker.bindTooltip(`<b>${driver.vehicleType}</b><br/>${hasTask ? "جارٍ التوصيل" : "متوقف"}`, { sticky: true });
      });
    });
  }, [data, filter]);

  const isDriver = (s: ZoneFeature | DriverFeature): s is DriverFeature => "lat" in s;
  const isZone = (s: ZoneFeature | DriverFeature): s is ZoneFeature => "boundary" in s;
          ...
          <div className="map-panel" dir="rtl">
            ...
            {selectedType === "driver" && isDriver(selected) && (
              <>
                ...
                <div className="panel-stats">
                  <div className="panel-stat">
                    <div className="panel-stat-val" style={{ fontSize: 18 }} />
