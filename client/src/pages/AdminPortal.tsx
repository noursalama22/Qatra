...
export default function AdminPortal() {
  ...
  return (
    <div className="portal admin-portal" dir="rtl">
      {/* Header */}
      <div className="portal-header" style={{ background: "linear-gradient(135deg,#1e3a5f,#1e40af)" }}>
      ...
      <div className="portal-tabs">
        ...
        <button className={`ptab ${tab === "ngos" ? "ptab-active" : ""}`} onClick={() => setTab("ngos")}>
          ❤️ المنظمات {pendingNgos.length > 0 && <span className="ptab-badge">{pendingNgos.length}</span>}
        </button>
        ...
        {tab === "overview" && stats && (
          <>
            <div className="stat-grid">
              {[
                { label: "مناطق نشطة", val: stats.activeZones, icon: "🗺️", color: "#2563eb" },
                { label: "منظمات معتمدة", val: stats.approvedNgos, icon: "❤️", color: "#10b981" },
                { label: "مزودون معتمدون", val: stats.approvedProviders, icon: "🏢", color: "#8b5cf6" },
            ...
            {totalPending > 0 && (
              <div className="pending-banner">
                <div className="pending-banner-icon">⚠️</div>
                <div>
            ...
            <div className="config-section">
              <div className="section-title">⚙️ إعدادات النظام</div>
              ...
              <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 12 }}>* التعديل على الإعدادات متاح في الإصدارات القادمة</div>
            </div>
          </>
        )}
                ...
                {pendingNgos.map(n => (
                  <div key={n.id} className="approval-card">
                    <div className="approval-info">
                      <div className="approval-name">❤️ {n.orgName}</div>
            ...
            <div className="approval-section">
              ...
              {ngos.filter(n => n.status === "approved").map(n => (
                <div key={n.id} className="actor-card">
                  <div className="actor-icon" style={{ background: "#dcfce7", color: "#16a34a" }}>❤️</div>
              ...
              {ngos.filter(n => n.status === "rejected").map(n => (
                <div key={n.id} className="actor-card" style={{ opacity: 0.6 }}>
                  <div className="actor-icon" style={{ background: "#fee2e2", color: "#dc2626" }}>❤️</div>
                  <div className="actor-info">
                    <div className="actor-name">{n.orgName}</div>
