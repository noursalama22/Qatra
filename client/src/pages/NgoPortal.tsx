...
function priorityColor(score: number) {
  return score > 70 ? "#dc2626" : score > 50 ? "#ea580c" : score > 30 ? "#f59e0b" : "#14b8a6";
}
      ...
      zones.forEach(zone => {
        ...
        if (zone.boundary && zone.boundary.length > 2) {
          const poly = L.polygon(zone.boundary, {
            ...
            fillColor: isSelected ? "#0ea5e9" : color,
        ...
        <div className="ngo-sched-split">
          ...
          <div className="ngo-cal-panel">
            {!calZone ? (
              <div className="ngo-cal-empty">
                ...
                <div style={{ fontSize: 16, fontWeight: 700, color: "#12384f", marginBottom: 8 }}>
                  اختر حياً من الخريطة
                </div>
                <div style={{ fontSize: 13, color: "#6b8aa0", lineHeight: 1.6, maxWidth: 280, textAlign: "center" }}>
                ...
                <div className="ngo-cal-zone-bar">
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 15, fontWeight: 800, color: "#12384f" }}> {calZone.name}</span>
                      ...
                      {days.map(d => {
                        ...
                        return (
                          <div key={dayKey(d)} className={`ngo-cal-day-head ${isToday ? "ngo-cal-day-today" : ""}`}>
                            ...
                            {isToday && <div style={{ fontSize: 9, color: "#0ea5e9", fontWeight: 700, marginTop: 2 }}>اليوم</div>}
                          </div>
                        );
                      })}
                    </div>
                          ...
                          if (state === "locked") {
                            return (
                              <div key={dk} className="ngo-cal-cell ngo-cell-locked"
                                ...
                                <div style={{ fontSize: 9, color: "#8eb5c8", marginTop: 2 }}>محجوز</div>
                              </div>
                            );
                          }
                          ...
                          if (state === "mine" && task) {
                            ...
                            return (
                              <div key={dk} className="ngo-cal-cell ngo-cell-mine" style={{ background: col + "18", borderColor: col + "55" }}>
                                ...
                                <div style={{ fontSize: 9, color: "#31576b", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          ...
                          if (isPast) {
                            return (
                              <div key={dk} className="ngo-cal-cell ngo-cell-past">
                                <div style={{ fontSize: 9, color: "#b9d7e6" }}>—</div>
                              </div>
                            );
                          }
          ...
          <div className="ngo-map-panel-new">
            ...
            <div className="ngo-map-legend">
              <div style={{ fontWeight: 700, fontSize: 11, color: "#12384f", marginBottom: 6 }}>خريطة الأولوية</div>
              {([ ["#dc2626","عالية جداً"], ["#ea580c","عالية"], ["#f59e0b","متوسطة"], ["#14b8a6","منخفضة"] ] as const).map(([c, l]) => (
                <div key={l} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "#31576b", marginBottom: 3 }}>
                  <span style={{ width: 10, height: 10, borderRadius: 3, background: c, flexShrink: 0 }} />{l}
                </div>
              ))}
              <div style={{ fontSize: 10, color: "#8eb5c8", marginTop: 6, lineHeight: 1.4 }}>انقر على الحي لعرض التقويم</div>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="ngo-sched-drawer" dir="rtl">
            ...
            {submitted ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", padding: 40, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 20, fontWeight: 800, color: "#0891b2", marginBottom: 8 }}>تم تأكيد الجدول!</div>
                <div style={{ fontSize: 14, color: "#6b8aa0" }}>المبلغ محجوز في حساب الضمان ريثما يُثبَت التسليم</div>
              </div>
            ) : (
              <>
                {/* Drawer header */}
                <div className="ngo-drawer-head">
                  <div>
                    <div style={{ fontSize: 11, color: "#6b8aa0", marginBottom: 3, textTransform: "uppercase", letterSpacing: "0.5px" }}>جدولة توزيع</div>
                    <div style={{ fontSize: 17, fontWeight: 800, color: "#12384f" }}> {calZone.name}</div>
                    <div style={{ fontSize: 12, color: "#6b8aa0", marginTop: 3 }}>
                ...
                <div className="ngo-drawer-section">
                  ...
                  {qty > 0 && (
                    <div style={{ fontSize: 12, color: "#6b8aa0", marginTop: 6, display: "flex", gap: 12 }}>
                      <span> {qty.toLocaleString()} لتر</span>
                      <span style={{ color: "#8eb5c8" }}>≈ {(qty / 1000).toFixed(1)} طن</span>
                    </div>
                  )}
                </div>
                        ...
                        return (
                          <div key={p.id}
                            ...
                            <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                              <div style={{ width: 44, height: 44, background: isSelected ? "#dff4ff" : "#eef8fd", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}></div>
                              <div style={{ flex: 1 }}>
                                <div style={{ fontWeight: 700, fontSize: 14, color: "#12384f" }}>{p.companyName}</div>
                                <div style={{ fontSize: 12, color: "#6b8aa0", marginTop: 2 }}>{p.ext.source}</div>
                              </div>
                              <div style={{ textAlign: "center" }}>
                                <div style={{ fontSize: 18, fontWeight: 800, color: isSelected ? "#0284c7" : "#0891b2" }}>
                                  ${total.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                                </div>
                                <div style={{ fontSize: 10, color: "#8eb5c8" }}>إجمالي العقد</div>
                              </div>
                            </div>

                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: 12, paddingTop: 12, borderTop: `1px solid ${isSelected ? "#bae6fd" : "#eef8fd"}` }}>
                              <div style={{ textAlign: "center" }}>
                                <div style={{ fontSize: 13, fontWeight: 700, color: "#f59e0b" }}>
                                  {"".repeat(Math.floor(p.ext.trustScore))} {p.ext.trustScore}
                                </div>
                                <div style={{ fontSize: 9, color: "#8eb5c8", marginTop: 2 }}>{p.ext.completedJobs} مهمة ناجحة</div>
                              </div>
                              <div style={{ textAlign: "center" }}>
                                <div style={{ fontSize: 14, fontWeight: 700, color: "#12384f" }}>{p.ext.freeFleet}</div>
                                <div style={{ fontSize: 9, color: "#8eb5c8", marginTop: 2 }}>شاحنات متفرغة</div>
                              </div>
                              <div style={{ textAlign: "center" }}>
                                <div style={{ fontSize: 13, fontWeight: 700, color: "#12384f" }}>${p.ext.pricePerLiter}</div>
                                <div style={{ fontSize: 9, color: "#8eb5c8", marginTop: 2 }}>/ لتر</div>
                              </div>
                            </div>
                          </div>
                        );
                ...
                {drawerProvider && qty > 0 && (
                  <div className="ngo-escrow-notice">
                    <div style={{ fontWeight: 700, fontSize: 13, color: "#12384f", marginBottom: 4 }}> ملخص العقد</div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#31576b", padding: "4px 0" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#31576b", padding: "4px 0", borderTop: "1px solid #d8eef8", marginTop: 4, paddingTop: 8 }}>
                    ...
                    <div style={{ marginTop: 10, padding: "8px 12px", background: "#fef3c7", borderRadius: 8, fontSize: 12, color: "#0f5f8d" }}>
